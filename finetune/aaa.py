...
if __name__ == "__main__":
    ...
    for filename in os.listdir(json_dir):
        if filename.endswith(".json") and filename != "output.json" and filename != "femiclear.com.json":
            try:
                # Read the JSON file
...
The next text after the image appears: {image["next_text"]}\n\n"""
                    ...
                    with open(f"./human-data-not-filtered.jsonl", "a") as file:
                        json.dump({"web_url": filename, "img_url": image["src"], "role": image["role"]}, file)
                        file.write("\n")
